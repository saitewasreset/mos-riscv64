...
static int pipe_read(struct Fd *fd, void *buf, uint32_t n, uint32_t offset);
static int pipe_stat(struct Fd *, struct Stat *);
static int pipe_write(struct Fd *fd, const void *buf, uint32_t n,
                      uint32_t offset);
...
struct Pipe {
    uint32_t p_rpos;         // read position
    uint32_t p_wpos;         // write position
...
int pipe(int pfd[2]) {
    ...
    if ((r = fd_alloc(&fd0)) < 0 ||
        (r = syscall_mem_alloc(0, fd0,
                               PTE_V | PTE_RW | PTE_USER | PTE_LIBRARY)) < 0) {
        goto err;
    }

    if ((r = fd_alloc(&fd1)) < 0 ||
        (r = syscall_mem_alloc(0, fd1,
                               PTE_V | PTE_RW | PTE_USER | PTE_LIBRARY)) < 0) {
        goto err1;
    }
    ...
    va = fd2data(fd0);
    if ((r = syscall_mem_alloc(0, (void *)va,
                               PTE_V | PTE_RW | PTE_USER | PTE_LIBRARY)) < 0) {
        goto err2;
    }
...
static int _pipe_is_closed(struct Fd *fd, struct Pipe *p) {
    ...
    uint32_t before_env_run = env->env_runs;

    while (1) {
        fd_ref = pageref(fd);
        pipe_ref = pageref(p);

        uint32_t after_env_run = env->env_runs;
        if (after_env_run == before_env_run) {
            break;
        }
    }
 ...
 */
static int pipe_read(struct Fd *fd, void *vbuf, uint32_t n, uint32_t offset) {
    uint32_t i;
    struct Pipe *p;
    char *rbuf;
    ...
    while (i < n) {
        ...
        // 未读取到目标长度，说明**当时**管道中没有数据
        // 由于并发，**现在**管道中可能有数据
        // 若**现在**管道中可能有数据，到循环头部，读取剩余数据

        while (p->p_rpos == p->p_wpos) {
            // 管道中“大概”没有数据
            // 由于并发性，执行到这一行时，管道中可能有数据
            if ((i >= 1) || (_pipe_is_closed(fd, p) != 0)) {
                return (int)i;
            }
 ...
 *   The parameter 'offset' isn't used here.
 */
static int pipe_write(struct Fd *fd, const void *vbuf, uint32_t n,
                      uint32_t offset) {
    int i;
    struct Pipe *p;
    char *wbuf;
    ...
    while (i < n) {
        // 仍然可以写入数据
        // p_rpos一定<=p_wpos，相减一定>=0
        while ((p->p_wpos - p->p_rpos < PIPE_SIZE) && (i < n)) {
            wbuf[p->p_wpos % PIPE_SIZE] = buf[i];
        ...
        // 未写入到目标长度，说明**当时**管道已满
        // 由于并发，**现在**管道可能未满
        // 若**现在**管道未满，到循环头部，写入剩余数据

        while (p->p_wpos - p->p_rpos == PIPE_SIZE) {
            // 管道“大概”已满
            // 由于并发性，执行到这一行时，管道可能未满

            if (_pipe_is_closed(fd, p) != 0) {
                return (int)i;
            }
