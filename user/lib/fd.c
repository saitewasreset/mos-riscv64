...
int fd_lookup(int fdnum, struct Fd **fd) {
    ...
    if (syscall_get_physical_address((void *)va) != 0) { // the fd is used
        *fd = (struct Fd *)va;
        return 0;
    }
...
int fd2num(struct Fd *fd) { return ((u_reg_t)fd - FDTABLE) / PTMAP; }
 ...
 *
 *   先复制file data，再复制fd，防止管道ref(read) + ref(write) == ref(data)
 *   结论判断管道是否已经关闭由于竞争失效
 *
 * Precondition：
...
int dup(int oldfdnum, int newfdnum) {
    ...
    if (syscall_get_physical_address(ova) != 0) {
        for (u_reg_t offset = 0; offset < PDMAP; offset += PTMAP) {
            void *current_old = (void *)((u_reg_t)ova + offset);
            void *current_new = (void *)((u_reg_t)nva + offset);
            if (syscall_get_physical_address(current_old) != 0) {
                if ((r = syscall_mem_map(0, current_old, 0, current_new,
...
err:
    ...
    for (u_reg_t i = 0; i < PDMAP; i += PTMAP) {
        panic_on(syscall_mem_unmap(0, (void *)((u_reg_t)nva + i)));
    }
...
int read(int fdnum, void *buf, uint32_t n) {
    int r = 0;
 ...
 *      - 对于文件，由于进行的是缓存中的读取，总是会成功
 */
int readn(int fdnum, void *buf, uint32_t n) {
    int m, tot;
 ...
 */
int write(int fdnum, const void *buf, uint32_t n) {
    int r;
    struct Dev *dev;
    struct Fd *fd;
 ...
 *   - 若`oldfdnum`不对应有效的文件描述符（越界，未打开），返回-E_INVAL
 */
int seek(int fdnum, uint32_t offset) {
    int r;
    struct Fd *fd;
